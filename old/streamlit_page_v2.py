...
def filter_data():
    ...
    if st.session_state.data is not None:
        ...
        if st.session_state.current_index >= len(filtered):
            st.session_state.current_index = 0 if len(filtered) > 0 else 0
...
if st.session_state.filtered_data is not None and len(st.session_state.filtered_data) > 0:
    ...
    with col2:
        ...
        st.write(f"**Categoría 1:** {row['categoria_1']}")
        st.write(f"**Categoría 2:** {row['categoria_2']}")
        
        # Mostrar imagen si está disponible
        if 'ruta' in row and row['ruta']:
            st.write("**Imagen de referencia:**")
            
            # Verificar si existe un directorio de imágenes o permitir subir la imagen
            if 'image_dir' not in st.session_state:
                st.session_state.image_dir = None
                st.session_state.uploaded_images = {}
            
            # Primera vez mostrar selector de directorio de imágenes
            if st.session_state.image_dir is None:
                st.write("Selecciona la carpeta raíz de imágenes o sube la imagen específica:")
                image_dir = st.text_input("Carpeta raíz de imágenes:", value="")
                if image_dir and os.path.exists(image_dir):
                    st.session_state.image_dir = image_dir
                    st.success(f"Directorio de imágenes configurado: {image_dir}")
                
                # Opción para subir imagen específica
                uploaded_img = st.file_uploader("O sube la imagen específica:", type=["jpg", "jpeg", "png"], key=f"img_{row['id']}")
                if uploaded_img is not None:
                    st.session_state.uploaded_images[row['ruta']] = uploaded_img
            
            # Intentar cargar la imagen
            image_path = None
            image_loaded = False
            
            # Opción 1: Usar imagen subida
            if row['ruta'] in st.session_state.uploaded_images:
                try:
                    import io
                    from PIL import Image
                    image = Image.open(st.session_state.uploaded_images[row['ruta']])
                    st.image(image, caption=row['nombre_imagen'], use_column_width=True)
                    image_loaded = True
                except Exception as e:
                    st.error(f"Error al cargar la imagen subida: {str(e)}")
            
            # Opción 2: Buscar en directorio configurado
            if not image_loaded and st.session_state.image_dir is not None:
                try:
                    ...
                    if os.path.exists(image_path):
                        ...
                        # Ofrecer opción para subir la imagen específica si no se encuentra
                        st.write("Sube esta imagen específica:")
                        uploaded_img = st.file_uploader("Subir imagen:", type=["jpg", "jpeg", "png"], key=f"img_missing_{row['id']}")
                        if uploaded_img is not None:
                            st.session_state.uploaded_images[row['ruta']] = uploaded_img
                    ...
                    st.error(f"Error al acceder a la imagen: {str(e)}")
            
            # Si no se pudo cargar la imagen, mostrar información
            if not image_loaded:
                st.info(f"Imagen: {row['nombre_imagen']}")
                st.info(f"Ruta: {row['ruta']}")
                
                # Permitir cambiar el directorio de imágenes
                if st.button("Cambiar directorio de imágenes"):
                    st.session_state.image_dir = None
    ...
    with col2:
        ...
        if selected_index != st.session_state.current_index + 1:
